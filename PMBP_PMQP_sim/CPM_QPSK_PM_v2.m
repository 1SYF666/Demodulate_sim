...
%% ***** data:20250115 authoor:ShenYifu ****  %%
%% 
%{
    参考论文《复合调制信号盲分析技术研究》
    参考论文《复合调制信号的识别与处理研究》
...
pcm_pairs = reshape(pcm_bits, r, []); % 转换为 2 行矩阵，每列是两位
pcm_pairs_transposed = pcm_pairs.'; % 转置，每行代表一组
phase = zeros(1,symbolnum);
tic;
for i = 1 : symbolnum
    char_vec = pcm_pairs_transposed(i,:);
    if char_vec == [0,0]
        phase(i) = pi*5/4;
    elseif char_vec == [0,1]
        phase(i) = pi*7/4;
...
ta = toc;fprintf('耗时：%.4f秒\n',ta);

% QPSK 调制
method = 1;
if method == 0
    sqpsk = exp(1i*phase);
    rcos_pcm_s1 = repmat(sqpsk,sps,1);
    rcos_pcm_s2 = rcos_pcm_s1(:).';
    rcos_pcm_s = rcos_pcm_s2;
    N = length(rcos_pcm_s);
    t = (0 : N-1)/fs;
    s_QPSK = real(rcos_pcm_s .* exp(1i*2 * pi * fcsub * t));
elseif method == 1
    rcos_pcm_s1 = repmat(phase,sps,1);
    rcos_pcm_s2 = rcos_pcm_s1(:).';
    rcos_pcm_s = rcos_pcm_s2;
    N = length(rcos_pcm_s);
    t = (0 : N-1)/fs;
    s_QPSK = cos(2 * pi * fcsub * t + rcos_pcm_s);
end


% PM 调制
Kp = pi/2;
s_PCM_BPSK_PM = cos(2 * pi * fc * t + Kp * s_QPSK);
s_PCM_BPSK_PM1 = sin(2 * pi * fc * t + Kp * s_QPSK);
...
% figure;subplot(2,1,1);plot(cwtfrealabs);title("小波系数图");subplot(2,1,2);plot(estsignalabs);
% 最小间隔求符号速率
peakstemp = [];
flag = 1;
index = 1;
...
ta = toc;fprintf('耗时：%.4f秒\n',ta);
%% 绘制星座图
starmap = (I_PLL_D1+1i*Q_PLL_D1);
figure;scatter(real(starmap(100:end)),imag(starmap(100:end)));title("码元同步输出星座图");

%% 判决
dey = zeros(1,2*length(starmap));
for i = 1 : length(starmap)
    temp = starmap(i);
    if real(temp)>0 && imag(temp)>0
        dey(1+(i-1)*r : i*r) = [1,1];
    elseif real(temp)<0 && imag(temp)>0
        dey(1+(i-1)*r : i*r) = [1,0];
    elseif real(temp)<0 && imag(temp)<0
        dey(1+(i-1)*r : i*r) = [0,0];
    else
...
%% 误码率
len = length(pcm_bits);
errorbit = sum(dey(1:len)~=pcm_bits)/len;
fprintf("误比特率：%.4f\n",errorbit);
ta = toc;fprintf('耗时：%.4f秒\n',ta);
